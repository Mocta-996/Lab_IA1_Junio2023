...
hotel(33, 'El Bosque', 'Km 280', 5, 500.00, 700.00, 175.00, 5, 280).    
...
(
    ...
    OPCION == 4 -> menuClima;
    OPCION == 5 -> menuReportes;    
    OPCION == 6 -> write('¡Hasta pronto!'), nl, halt;
    write('Opcion invalida'), nl, menuInicio()
).
...
opcionesPresupuesto(PRESUPUESTO):-
(
    % Verificar el presupuesto
    PRESUPUESTO =< 1500 -> presupuestoBarato(PRESUPUESTO); 
    PRESUPUESTO > 3000 ->  presupuestoCaro(PRESUPUESTO);
...
(
    ...
    % mostrar resultados
    write('========================== Resultados ==========================  '), nl,
    write('Hotel: '), write(B_NOMBRE), nl,
    write('Departamento: '), write(B_DEPTONOMBRE), nl,
    write('Dirección: '), write(B_DIRECCION), nl,
...
opcionesIdioma(R_IDIOMA) :-
(
    R_IDIOMA == 'n' -> idiomaLocal();
    R_IDIOMA == 's' -> idiomaExtranjero();
    write('Respuesta inválida'), nl,
    menuIdioma
).
...
hotelCaro(CALIFICACION):-
(
    write('Ingrese un tiempo máximo para el viaje: '), nl,
    read(TIEMPO_MAX), nl,
    write(' ¿LLevara vehículo ?  (s/n):'), nl,
    read(VEHICULO), nl,
    write(' Tipo de habitación (escribir el número):
    ...
    write('¿Cuantos dias desea hospedarse?:'), nl,
    read(DIAS), nl,
    resultadoHotelCaro(CALIFICACION,TIEMPO_MAX,VEHICULO,TIPO_HABITACION,CANTIDAD_PERSONAS,DIAS)
).
...
% ----- OPCIONES MENU REPORTES ----- %
opcionMenuReportes(OPCION) :-
(
    OPCION == 1 -> reporte1();
    OPCION == 2 -> reporte2();
    OPCION == 3 -> reporte3();
    ...
    OPCION == 8 -> reporte8();
    OPCION == 9 -> reporte9();
    write('Opcion invalida'), nl, menuReportes()
).
